...
def findInProduct(keyName, textToSearch):
    ...
    for key in keyName:
        find = False
        keyPart = key.split(' ')
        permutazioni = list(permutations(keyPart))
        for permutazione in permutazioni:
            if ' '.join(permutazione) in textToSearch.lower() or ''.join(
                    permutazione) in textToSearch:  # controllo sia con gli spazi che senza gli spazi
                keyName.remove(key)
                find = True
                break
...
def getProducts():
    ...
    if content_type != 'application/json;charset=UTF-8':
        ...
        if not findMore:
            ...
            mainDriver.get(linkAmazon + linkNextPage)
        ...
        if numOfItems != 0:
            ...
            EC.presence_of_all_elements_located((By.XPATH, '//span[@class="s-pagination-strip"]/span[@class="s-pagination-item s-pagination-next s-pagination-button s-pagination-separator"]')))
                linkNextPage = linkNextPage.get_attribute("href")
